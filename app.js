...
let UIController = (function () {
  ...
  let formatNumber = function (num, type) {
    ...
    if (int.length > 3 && int.length < 6) {
      ...
      int =
        int.substr(0, int.length - 5) +
        "," +
        int.substr(int.length - 5, int.length - 5) +
        "," +
        int.substr(int.length - 3, int.length);
    }
  ...
  return {
    ...
    addItemToUI: function (obj, type) {
      ...
      document
        .querySelector(classSelect)
        .insertAdjacentHTML("beforeend", replacedHtml);
    },
    ...
    displayBudgetOnUI: function (object) {
      ...
      document.querySelector(DOMstrings.expensesLabel).textContent =
        formatNumber(object.totalExpenses, type);
...
let controller = (function (budgetController, UIController) {
  ...
  let setupEventListeners = function () {
    ...
    document
      .querySelector(DOM.classSelectContainer)
    ...
    document
      .querySelector(DOM.darkModeToggle)
